...
def SPEAKER_INFORMATION_WEIGHTS(layer): 
    return generate_matrix_from_index(layer)
...
class KNeighborsVC(nn.Module):
    ...
    def __init__(self,
        ...
        weights = SPEAKER_INFORMATION_WEIGHTS(layer)
        self.weighting = torch.tensor(weights, dtype=torch.float32 if device=="mps" else None, device=device)[:, None]
        # load hifigan
